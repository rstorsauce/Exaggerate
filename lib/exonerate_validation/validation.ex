defmodule Exonerate.Validation.Helper do
  ...
  """
  def checkfn(:integer), do: (quote do is_integer(val) end)
  def checkfn(:format),  do: (quote do val in ["date-time", "email", "hostname", "ipv4", "ipv6", "uri"] end)
  def checkfn(:string),  do: (quote do is_binary(val) end)
  def checkfn(:number),  do: (quote do is_number(val) end)
  def checkfn(:map),     do: (quote do is_map(val) end)
  def checkfn(:addprop), do: (quote do (is_map(val) or is_boolean(val)) end)
  def checkfn(:list),    do: (quote do is_list(val) end)
  def checkfn(:items),   do: (quote do is_map(val) or is_list(val) end)
  def checkfn(:boolean), do: (quote do is_boolean(val) end)
  def checkfn(:additem), do: (quote do (is_map(val) or is_boolean(val)) end)
  ...
  defmacro parameter(name, type, fortype, fallthrough \\ false) do
    ...
    quote do
      def validate_kv({unquote(name), val}, map = %{"type" => unquote(fortype)}, _) when unquote(checker),                       do: unquote(fallthru)
      def validate_kv({unquote(name), val}, map = %{"type" => type}, _)             when unquote(checker) and is_binary(type),   do: {:error, unquote(errmsg)}
      def validate_kv({unquote(name), val}, map = %{"type" => typelist}, _)         when unquote(checker) and is_list(typelist), do: if unquote(fortype) in typelist, do: unquote(fallthru), else: {:error, unquote(errmsg)}
  ...
  defmacro numeric_parameter(name, type) do
    ...
    valmsg = "#{name} can't be non-#{type} value"
    quote do
      def validate_kv({unquote(name), val}, %{"type" => "integer"}, _)        when unquote(checker),                       do: :ok
      def validate_kv({unquote(name), val}, %{"type" => "number"}, _)         when unquote(checker),                       do: :ok
      def validate_kv({unquote(name), val}, %{"type" => type}, _)             when unquote(checker) and is_binary(type),   do: {:error, unquote(errmsg)}
      def validate_kv({unquote(name), val}, %{"type" => typelist}, _)         when unquote(checker) and is_list(typelist), do: if ("integer" in typelist) || ("number" in typelist), do: :ok, else: {:error, unquote(errmsg)}
...
defmodule Exonerate.Validation do
  ...
  def validate_required(parent = %{"properties" => props}, req) do
    Enum.map(req, fn key -> if key in Map.keys(props), do: :ok, else: {:error, "required item #{key} not in properties of #{inspect parent}"} end)
      |> error_reduction
  end
  def validate_required(parent, _), do: {:error, "required properties requires properties, missing in #{inspect parent}"}
  ...
  def validate_dependencies(_parent, depmap), do: Enum.map(depmap, fn {_k,v} -> check_dependency(v) end) |> error_reduction
  #array of strings case:
  def check_dependency(deplist) when is_list(deplist) do
    if Enum.all?(deplist, &is_binary/1), do: :ok, else: {:error, "non-string item in dependency list: #{inspect deplist}"}
  end
  def check_dependency(depmap) when is_map(depmap), do: validate(depmap, false)  #or recursively check that it's a schema.
  def check_dependency(value), do: {:error, "unrecognized value in dependency list #{inspect value}"}

  def validate_patternProperties(parent, pp), do: Enum.map(pp, fn {_k, v} -> validate(v, false) end) |> error_reduction

  def validate_items(_parent, obj) when is_map(obj), do: validate(obj, false)
  def validate_items(_parent, arr) when is_list(arr), do: Enum.map(arr, &validate(&1, false)) |> error_reduction
