defmodule Exaggerate.Codesynth.Routesynth do
    ...
    def get_alt_response(http_code = "2" <> _xx, response_map = %{"content" => _content}) do
      #assign the error description (which is a required parameter)
      %{"description" => response_desc} = response_map
      ...
      """
      #handles the #{response_desc} error.
      {:ok, #{http_code}, content} -> send_formatted(conn, #{http_code}, content)
      """
    end
    def get_alt_response(_,_), do: nil

    ##############################################################################
    ## error responses

    def get_error_response(error_val = "4" <> _xx, error_map = %{"content" => _content}) do
      #assign the error description (which is a required parameter)
      %{"description" => error_desc} = error_map
      #check that this error_val corresponds to a http response number.
      {_number, _} = Integer.parse(error_val)
      #figure out content validation later, using compile-time schemas.
      """
      #handles the #{error_desc} error.
      {:error, #{error_val}, content} -> send_formatted(conn, #{error_val}, content)
      """
    end
    def get_error_response(error_val = "4" <> _xx, _error_map = %{"description" => error_desc}) do
      #check that this error_val corresponds to a http response number.
      {_number, _} = Integer.parse(error_val)
      #a simple error reporting, using the description.
      "{:error, \"#{error_desc}\"} -> send_formatted(conn, #{error_val}, %{\"#{error_val}\" => \"#{error_desc}\"})"
    end
    def get_error_response(_, _), do: nil
    ...
    def build_route(verb, route, route_def, routemodule) when is_atom(verb) and is_binary(route) and is_map(route_def) do
      ...
      alt_codes    = get_responses(route_def["responses"], &get_alt_response/2)
      error_codes  = get_responses(route_def["responses"], &get_error_response/2)
