defmodule Exaggerate.Validation.Helpers do
  ...
  def required_call_generator(required_param) do
    ...
    quote do
      def is_valid?(api_map, [unquote(required_param) | tail], optional_list) do
        if Map.has_key?(api_map, unquote(key_string)) do
          Exaggerate.Validation.error_search([unquote(validation_fn)(api_map[unquote(key_string)]),
                                              is_valid?(api_map, tail, optional_list)])
        else
  ...
  def optional_call_generator(optional_param) do
    ...
    quote do
      def is_valid?(api_map, [], [unquote(optional_param) | tail]) do
        if Map.has_key?(api_map, unquote(key_string)) do
          Exaggerate.Validation.error_search([unquote(validation_fn)(api_map[unquote(key_string)]),
                                              is_valid?(api_map, [], tail)])
        else
          is_valid?(api_map, [], tail)
        end
      end
  ...
  @doc """
    for a validation module, creates the is_valid? function.  A validation module
    usually will implement this function, but for some corner cases, a special
    is_valid function may be hand-written.
  """
  defmacro validate_keys(required, optional) do
    ...
    quote do
      def is_valid?(api_map), do: is_valid?(api_map, unquote(required), unquote(optional))
      def is_valid?(api_map,[],[]), do: unquote(extra_results)
  ...
  defmacro object_parameter(parameter, modname \\ nil) do
    ...
    modname = modname || parameter |> Atom.to_string
                                   ...
                                   |> String.to_atom
    quote do
      def unquote(vfunc)(obj) when is_map(obj), do: Module.concat(Exaggerate.Validation, unquote(modname)).is_valid?(obj)
  ...
  defmacro object_parameter(parameter, modname, alt_mod) do
    ...
    quote do
      def unquote(vfunc)(obj) when is_map(obj) do
         primary_obj = Module.concat(Exaggerate.Validation, unquote(modname)).is_valid?(obj)
         alt_obj =     Module.concat(Exaggerate.Validation, unquote(alt_mod)).is_valid?(obj)
  ...
  defmacro pass_validate do
    quote do
      def is_valid(_) do
        IO.puts("warning: #{__MODULE__} is not validated")
        {:ok}
      end
    end
  end
